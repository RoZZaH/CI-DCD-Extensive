...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, form.password.data):
            ...
            return redirect(next_page) if next_page else redirect(url_for("public.show_tourdates"))
        else:
...
def logout():
    logout_user()
    return redirect(url_for("public.show_tours"))
...
@user.route("/add_band", methods=("GET", "POST"))
def add_band():
    form = BandDetailsForm()
    # if request.method == "POST":
    #   return request.form #["hometown-origin_town"]
    if form.validate_on_submit():
        if form.picture.data:
            output_size = (400,400)
    ...
    form.hometown.origin_town.choices = [(otown.town, otown.town) for otown in Towns.objects(county="Antrim")]
    genres = list_genres()
    return render_template("band_details.html", otype="band", form=form, genrelist=genres)
...
@user.route("/bands")
@user.route("/bands/")
@user.route("/bands/manage")
@user.route("/band/manage/")
def manage_bands():
    bands = Band.objects.order_by('org_title')

    return render_template("manage_bands.html", bands=bands)

@user.route("/bands/manage/<band_name>")
def manage_band(band_name):
    band = Band.objects(org_title = band_name)
    
@user.route("/bands/manage/<band_name>/tours")
...
@user.route("/bands/tours/")
def list_tours():
    tours = Tour.objects()
    #return jsonify(tours)# render_template("manage_tours.html", tours=tours)
    return render_template("manage_tours.html", tours=tours)

@user.route("/tours/edit/<tour_id>", methods=("GET","POST"))
